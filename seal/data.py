...
class BeirQueryIterator(QueryIterator):

    def get_query(self, id_):
        return self.topics[id_]['text']

    @classmethod
    def from_topics(cls, topics_path: str, queries_path: Optional[str]):
        topics = {}
        order = []

        if queries_path is None:
            queries_path = pathlib.Path(topics_path).absolute().parent.parent / 'queries.jsonl'
            queries_path = str(queries_path)

        with jsonlines.open(queries_path) as fin:
            for instance in fin:
                topics[instance['_id']] = instance
...
def get_query_iterator(topics_path: str, topics_format: TopicsFormat, queries_path: Optional[str] = None):
    mapping = {
        ...
        TopicsFormat.BEIR: BeirQueryIterator,
        TopicsFormat.DPR: DprQueryIterator,
        TopicsFormat.DPR_QAS: DprQueryQasIterator,
        TopicsFormat.NQ: NqQueryIterator,
    }
    if topics_format == TopicsFormat.BEIR:
        return mapping[topics_format].from_topics(topics_path, queries_path)
    else:
...
class BeirWriter(OutputWriter):
    ...
    def __enter__(self):
        ret = super().__enter__()
        self._file.write(f'query-id\tcorpus-id\tscore\n')
        return ret

    def write(self, topic: str, hits: list):
        for docid, rank, score, _ in self.hits_iterator(hits):
            self._file.write(f'{topic}\t{docid}\t{score}\n')
